...
def run_episode(env, policy, scaler, animate=False):
    ...
    while not done:
        ...
        obs, reward, done, _ = env.step(np.squeeze(action, axis=0))################why axis =0?
        #env.render()################################################
...
################################################################################
        #if step_num%1000==0:
                       #policy.saver.save(policy.sess,"../p-model.ckpt")
                       #val_func.saver.save(val_func.sess,"../vf-model.ckpt")
        if step_num%5000==0:
                       w=str(step_num)
                       policy.saver.save(policy.sess,'../'+w+'-p-model.ckpt')
                       val_func.saver.save(val_func.sess,'../'+w+'-vf-model.ckpt')
                       rewards_result=trajectories[19]['rewards']
                       #f=open('/host/home/human/modcloud/trpo/w+'reward'.txt','a')
                       #f.write(str([trajectories[19]['rewards'] ]))
                       #f.close()
                       #np.savetxt("/host/home/human/modcloud/trpo/w+'r.txt'",rewards_result)
                       np.savetxt('../'+w+'r.txt',rewards_result)
################################################################################
        if killer.kill_now:
